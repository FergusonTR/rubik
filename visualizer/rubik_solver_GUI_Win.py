...
############ GUI COMPONENTS ############
        ...
        for y in range(6):
            for x in range(8):
                if y in range(2,4) or x in range(2,4):
                    button = Button(f, width=5, height=3, font=("Courier", 11))
                    #if (x == 3 and y == 5):
    ...
    def create_color_picker(self, f):
        self.label = Label(f,text="Pick colors to edit the cube:", font=("Courier", 12), bg="#FFFFFF")
        self.label.pack(side=LEFT)

        for c in self.colors.keys():
            button = Button(f, width=self.btnColor_w_defaut, height=self.btnColor_h_defaut, font=("Courier", 12))
    ...
    def create_controller(self, f):
        button = Button(f, text="Reset", command=self.btnReset_clicked, font=("Courier", 12))
        button["bg"] = "#FFFFFF"
        button.pack(side=LEFT)

        button = Button(f, text="Solve", command=self.solve_rubik, font=("Courier", 12))
        button["bg"] = "#FFFFFF"
        button.pack(side=LEFT)

        button = Button(f, text="Demo", command=self.show_demo, font=("Courier", 12))
        button["bg"] = "#FFFFFF"
        button.pack(side=LEFT)

        button = Button(f, text="About", command=self.show_about, font=("Courier", 12))
        button["bg"] = "#FFFFFF"
        button.pack(side=LEFT)

        self.lblInfo = Label(f, font=("Courier", 12), bg="#FFFFFF")
    ...
    def create_solution_panel(self, f):
        self.lblSol = Label(f, text="Solving steps:", font=("Courier", 12))
        self.lblSol.pack(side=LEFT)
        self.lblSol["text"] = ""

        button = Button(f, command=self.btnPrevMove_clicked, font=("Courier", 12))
        button.pack(side=LEFT)
        self.sol_left = button
        self.sol_left["borderwidth"] = 0;

        button = Button(f, command=self.btnNextMove_clicked, font=("Courier", 12))
        button.pack(side=LEFT)
        self.sol_right = button
        self.sol_right["borderwidth"] = 0;

        for i in range(15):
            button = Button(f, state=DISABLED, width=2, font=("Courier", 12))
            button.pack(side=LEFT)
...
################### INIT ###################
    def __init__(self, root):
        ...
        fColor = Frame(root, height=75, width=660, bg="#FFFFFF")
        fColor.pack_propagate(0)
        fColor.pack()
        self.create_color_picker(fColor)

        fRubik = Frame(root, height=400, width=20, bg="#E2E2E2")
        fRubik.pack_propagate(0) # don't shrink
        fRubik.pack()
        self.create_rubik(fRubik)

        fSol = Frame(root, height=60, width=660, bg="#E2E2E2")
        fSol.pack_propagate(0)
        fSol.pack()
        self.create_solution_panel(fSol)

        fController = Frame(root, height=60, width=660, bg="#FFFFFF")
